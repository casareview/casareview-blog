...
export type Post = {
  ...
  category: 'eletrodomesticos' | 'cozinha' | 'casa' | 'reviews'
  tags?: Array<string>
  coverImage: {
    asset?: {
      _ref: string
...
// Query: *[_type == "post" && defined(slug.current)] | order(date desc, _updatedAt desc) {      _id,  "status": select(_originalId in path("drafts.**") => "draft", "published"),  "title": coalesce(title, "Untitled"),  "slug": slug.current,  excerpt,  metaTitle,  metaDescription,  category,  tags,  coverImage {    asset,    alt  },  "date": coalesce(date, _updatedAt),  "author": author->{firstName, lastName, picture},  }
export type AllPostsQueryResult = Array<{
  ...
  category: 'casa' | 'cozinha' | 'eletrodomesticos' | 'reviews'
...
// Variable: postByCategoryQuery
// Query: *[_type == "post" && category == $category && slug.current == $slug] [0] {    content[]{    ...,    markDefs[]{      ...,        _type == "link" => {    "page": page->slug.current,    "post": post->slug.current  }    }  },      _id,  "status": select(_originalId in path("drafts.**") => "draft", "published"),  "title": coalesce(title, "Untitled"),  "slug": slug.current,  excerpt,  metaTitle,  metaDescription,  category,  tags,  coverImage {    asset,    alt  },  "date": coalesce(date, _updatedAt),  "author": author->{firstName, lastName, picture},  }
export type PostByCategoryQueryResult = {
  content: Array<{
    children?: Array<{
      marks?: Array<string>
      text?: string
...
// Variable: postPagesCategorySlugs
// Query: *[_type == "post" && defined(slug.current) && defined(category)]  {"category": category, "slug": slug.current}
export type PostPagesCategorySlugsResult = Array<{
  category: 'casa' | 'cozinha' | 'eletrodomesticos' | 'reviews'
  slug: string
}>
// Variable: postsByCategoryQuery
// Query: *[_type == "post" && category == $category && defined(slug.current)] | order(date desc, _updatedAt desc) {      _id,  "status": select(_originalId in path("drafts.**") => "draft", "published"),  "title": coalesce(title, "Untitled"),  "slug": slug.current,  excerpt,  metaTitle,  metaDescription,  category,  tags,  coverImage {    asset,    alt  },  "date": coalesce(date, _updatedAt),  "author": author->{firstName, lastName, picture},  }
