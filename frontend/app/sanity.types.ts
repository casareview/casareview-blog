...
// Query: *[_type == "post" && defined(slug.current)] | order(date desc, _updatedAt desc) {      _id,  "status": select(_originalId in path("drafts.**") => "draft", "published"),  "title": coalesce(title, "Untitled"),  "slug": slug.current,  excerpt,  metaTitle,  metaDescription,  "category": category-> {              "slug": slug.current                  },  tags,  coverImage {    asset,    alt  },  "date": coalesce(date, _updatedAt),  "author": author->{firstName, lastName, picture},  }
...
// Query: *[_type == "post" && category->slug.current == $category && slug.current == $slug] [0] {    content[]{      ...,      markDefs[]{        ...,          _type == "link" => {    "page": page->slug.current,    "post": post->slug.current  }      }    },      _id,  "status": select(_originalId in path("drafts.**") => "draft", "published"),  "title": coalesce(title, "Untitled"),  "slug": slug.current,  excerpt,  metaTitle,  metaDescription,  "category": category-> {              "slug": slug.current                  },  tags,  coverImage {    asset,    alt  },  "date": coalesce(date, _updatedAt),  "author": author->{firstName, lastName, picture},  }
...
// Query: *[_type == "post" && defined(slug.current) && defined(category)] {    "slug": slug.current,    "category": category->slug.current    }
export type PostPagesCategorySlugsResult = Array<{
  slug: string | null
  category: string | null
}>
// Variable: postsByCategoryQuery
// Query: *[_type == "post" && category->slug.current == $category && defined(slug.current)] | order(date desc, _updatedAt desc) {      _id,  "status": select(_originalId in path("drafts.**") => "draft", "published"),  "title": coalesce(title, "Untitled"),  "slug": slug.current,  excerpt,  metaTitle,  metaDescription,  "category": category-> {              "slug": slug.current                  },  tags,  coverImage {    asset,    alt  },  "date": coalesce(date, _updatedAt),  "author": author->{firstName, lastName, picture},  }
